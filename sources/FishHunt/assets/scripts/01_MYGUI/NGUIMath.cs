...
static public class NGUIMath
{
	...
	static public Vector3[] CalculateWidgetCorners (MYWidget w)
	{
	...
	{
		MYWidget[] widgets = trans.GetComponentsInChildren<MYWidget>(includeInactive) as MYWidget[];
	 ...
	 static public Bounds CalculateRelativeWidgetBounds (Transform root, Transform child, bool includeInactive = false)
	{
		...
		MYWidget[] widgets = child.GetComponentsInChildren<MYWidget>(includeInactive) as MYWidget[];
	...
	static public Bounds CalculateRelativeInnerBounds (Transform root, MYSprite sprite)
	{
		if (sprite.type == MYSprite.Type.Sliced)
		{
			Matrix4x4 toLocal = root.worldToLocalMatrix;
