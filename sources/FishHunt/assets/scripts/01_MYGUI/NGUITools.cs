...
//		for (int i = 0, imax = widgets.Length; i < imax; ++i) depth = Mathf.Max(depth, widgets[i].depth);
	...
	static public T AddWidget<T> (GameObject go) where T : MYWidget
	{
	...
	/// </summary>
/*
	static public UISprite AddSprite (GameObject go, MYAtlas atlas, string spriteName)
	{
		MYAtlas.Sprite sp = (atlas != null) ? atlas.GetSprite(spriteName) : null;
...
*/
	...
	{
		MYWidget w = t.GetComponent<MYWidget>();
