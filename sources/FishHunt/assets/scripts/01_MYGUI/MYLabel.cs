...
[ExecuteInEditMode]
[AddComponentMenu("MYGUI/UI/Label")]
public class MYLabel : MYWidget
{
	public enum Effect
	{
		None,
	...
	[HideInInspector][SerializeField] MYFont mFont;
	...
	[HideInInspector][SerializeField] MYFont.SymbolStyle mSymbols = MYFont.SymbolStyle.Uncolored;
	...
	public MYFont.SymbolStyle symbolStyle
	{
		get
		{
			return mSymbols;
	...
	/// <summary>
	/// MYLabel needs additional processing when something changes.
	/// </summary>
		...
		if (scale > 0f)
		{
			for (;;)
			{
				if (mPassword)
				{
					...
					mProcessedText = mFont.WrapText(mProcessedText, mMaxLineWidth / scale, mMaxLineCount, false, MYFont.SymbolStyle.None);
				}
...
//!!!		col.a *= mPanel.alpha;
		...
		{
			mFont.Print(processedText, col, verts, uvs, cols, mEncoding, mSymbols, MYFont.Alignment.Left, 0, mPremultiply);
		}
		else if (p == Pivot.Right || p == Pivot.TopRight || p == Pivot.BottomRight)
		{
			mFont.Print(processedText, col, verts, uvs, cols, mEncoding, mSymbols, MYFont.Alignment.Right,
				Mathf.RoundToInt(relativeSize.x * mFont.size), mPremultiply);
		}
		else
		{
			mFont.Print(processedText, col, verts, uvs, cols, mEncoding, mSymbols, MYFont.Alignment.Center,
				Mathf.RoundToInt(relativeSize.x * mFont.size), mPremultiply);
		}
...
//We need to override OnDrawGizmos for MYLabel components, because now it's shouldn't use relativeSize property 
#if UNITY_EDITOR
	void OnDrawGizmos ()
	{
		#if OLD_UNITY
