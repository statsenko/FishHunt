...
public abstract class MYWidget : MonoBehaviour
{
	public enum Pivot
	{
		TopLeft,
...
///!!!	protected UIPanel mPanel;
	...
	MYGeometry mGeom = new MYGeometry();
...
///!!!	public UIPanel panel { get { if (mPanel == null) CreatePanel(); return mPanel; } set { mPanel = value; } }
	...
	/// </summary>
/*	
	static public BetterList<MYWidget> Raycast (GameObject root, Vector2 mousePos)
	{
		BetterList<MYWidget> list = new BetterList<MYWidget>();
		MYCamera uiCam = MYCamera.FindCameraForLayer(root.layer);
		
		if (uiCam != null)
		{
			Camera cam = uiCam.cachedCamera;
			MYWidget[] widgets = root.GetComponentsInChildren<MYWidget>();
			...
			list.Sort(delegate(MYWidget w1, UIWidget w2) { return w2.mDepth.CompareTo(w1.mDepth); });
		}
		return list;
	}
*/	
	...
	static public int CompareFunc (MYWidget left, MYWidget right)
	{
...
*/
	...
	{
		#if UNITY_EDITOR
		if (GetComponents<MYWidget>().Length > 1)
		{
