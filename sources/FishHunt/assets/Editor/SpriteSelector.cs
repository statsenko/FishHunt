...
public class SpriteSelector : ScriptableWizard
{
	...
	public static void Show (MYAtlas atlas, string selectedSprite, Callback callback)
	{
	...
	public static void Show (MYAtlas atlas, MYSprite selectedSprite)
	{
		SpriteSelector comp = ScriptableWizard.DisplayWizard<SpriteSelector>("Select a Sprite");
		comp.mAtlas = atlas;
		...
		else
		{
			...
			while (offset < sprites.size)
			{
				GUILayout.BeginHorizontal();
				{
					...
					{
						MYAtlas.Sprite sprite = mAtlas.GetSprite(sprites[offset]);
