...
/// <summary>
/// Inspector class used to edit the MYAtlas.
/// </summary>

[CustomEditor(typeof(MYAtlas))]
public class MYAtlasInspector : Editor
{
	static public MYAtlasInspector instance;
	...
	{
		MYAtlas.Sprite sprite = (mAtlas != null) ? mAtlas.GetSprite(NGUISettings.selectedSprite) : null;
		if (sprite == null) return;

		MYSprite[] sprites = NGUITools.FindActive<MYSprite>();
		...
		MYLabel[] labels = NGUITools.FindActive<MYLabel>();

		foreach (MYLabel lbl in labels)
		{
			if (lbl.font != null && MYAtlas.CheckIfRelated(lbl.font.atlas, mAtlas) && lbl.font.UsesSprite(sprite.name))
			{
				MYFont font = lbl.font;
				lbl.font = null;
				lbl.font = font;
	...
	void OnSelectAtlas (MonoBehaviour obj)
	{
		if (mReplacement != obj)
		{
			...
			mAtlas.replacement = obj as MYAtlas;
	...
	public override void OnInspectorGUI ()
	{
		...
		{
			ComponentSelector.Draw<MYAtlas>(mAtlas.replacement, OnSelectAtlas);
		...
		if (!mConfirmDelete)
		{
			...
			if (mat != null)
			{
				...
				MYAtlas.Coordinates coords = (MYAtlas.Coordinates)EditorGUILayout.EnumPopup("Coordinates", mAtlas.coordinates);
			...
			else
			{
				...
				if (!mConfirmDelete && sprite != null)
				{
					...
					if (tex != null)
					{
						...
						if (mAtlas.coordinates == MYAtlas.Coordinates.Pixels)
						{
					...
					GUILayout.BeginHorizontal();
					{
						...
						if (GUILayout.Button("Clone Sprite"))
						{
							...
							MYAtlas.Sprite newSprite = new MYAtlas.Sprite();
