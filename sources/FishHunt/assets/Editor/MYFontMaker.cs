...
public class MYFontMaker : EditorWindow
{
	enum FontType
	{
		Bitmap,
	...
	void MarkAsChanged ()
	{
		...
		{
			List<MYLabel> labels = NGUIEditorTools.FindInScene<MYLabel>();
	...
	void OnGUI ()
	{
		...
		if (mType == FontType.Dynamic)
		{
			...
			if (NGUISettings.dynamicFont != null)
			{
				...
				if (go != null)
				{
					if (go.GetComponent<MYFont>() != null)
					{
		...
		else
		{
			...
			if (NGUISettings.fontData != null && NGUISettings.fontTexture != null)
			{
				...
				ComponentSelector.Draw<MYFont>("Select", NGUISettings.font, OnSelectFont);
				ComponentSelector.Draw<MYAtlas>(NGUISettings.atlas, OnSelectAtlas);
			}
			NGUIEditorTools.DrawSeparator();
		...
		if (create != 0)
		{
				...
				NGUISettings.fontName + " font with the currently selected values? This action can't be undone.", "Yes", "No"))
			{
				...
				{
					MYAtlasMaker.AddOrUpdate(NGUISettings.atlas, NGUISettings.fontTexture);
				}

				// Font doesn't exist yet
				if (go == null || go.GetComponent<MYFont>() == null)
				{
					...
					NGUISettings.font = go.AddComponent<MYFont>();
					...
					NGUISettings.font = go.GetComponent<MYFont>();
				}
				else
				{
					NGUISettings.font = go.GetComponent<MYFont>();
	...
	static void CreateFont (MYFont font, int create, Material mat)
	{
		if (create == 1)
		{
			// New dynamic font
