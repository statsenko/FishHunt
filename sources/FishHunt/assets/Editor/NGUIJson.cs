...
public class NGUIJson
{
	...
	public static void LoadSpriteData (MYAtlas atlas, TextAsset asset)
	{
		...
		List<MYAtlas.Sprite> oldSprites = atlas.spriteList;
		atlas.spriteList = new List<MYAtlas.Sprite>();
		...
		foreach (System.Collections.DictionaryEntry item in frames)
		{
			...
			foreach (MYAtlas.Sprite oldSprite in oldSprites)
			{
	...
	static int CompareSprites (MYAtlas.Sprite a, MYAtlas.Sprite b) { return a.name.CompareTo(b.name); }

	/// <summary>
	/// Copy the inner rectangle from one sprite to another.
	/// </summary>

	static void CopyInnerRect (MYAtlas.Sprite oldSprite, MYAtlas.Sprite newSprite)
	{
