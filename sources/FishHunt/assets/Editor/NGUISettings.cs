...
public class NGUISettings
{
	...
	static MYFont mFont;
	static MYAtlas mAtlas;
	static MYWidget.Pivot mPivot = MYWidget.Pivot.Center;
	...
	static void Load ()
	{
		...
		mFont			= GetObject("NGUI Font") as MYFont;
		mAtlas			= GetObject("NGUI Atlas") as MYAtlas;
		...
		mPivot			= (MYWidget.Pivot)EditorPrefs.GetInt("NGUI Pivot", (int)mPivot);
	...
	static public MYWidget.Pivot pivot
	{
		get
		{
			if (!mLoaded) Load();
