...
[CustomEditor(typeof(MYSprite))]
public class MYSpriteInspector : MYWidgetInspector
{
	protected MYSprite mSprite;
	...
	{
		mSprite = mWidget as MYSprite;
		ComponentSelector.Draw<MYAtlas>(mSprite.atlas, OnSelectAtlas);
	...
	protected override void DrawExtraProperties ()
	{
		...
		if (GetType() == typeof(MYSpriteInspector))
		{
			//GUILayout.BeginHorizontal();
			MYSprite.Type type = (MYSprite.Type)EditorGUILayout.EnumPopup("Sprite Type", mSprite.type);
		...
		if (mSprite.type == MYSprite.Type.Sliced)
		{
		...
		else if (mSprite.type == MYSprite.Type.Filled)
		{
			if ((int)mSprite.fillDirection > (int)MYSprite.FillDirection.Radial360)
			{
				mSprite.fillDirection = MYSprite.FillDirection.Horizontal;
				EditorUtility.SetDirty(mSprite);
			}

			MYSprite.FillDirection fillDirection = (MYSprite.FillDirection)EditorGUILayout.EnumPopup("Fill Dir", mSprite.fillDirection);
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
		...
		if (mSprite.atlas.coordinates == MYAtlas.Coordinates.Pixels)
		{
