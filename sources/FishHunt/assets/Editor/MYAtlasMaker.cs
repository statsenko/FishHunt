...
public class MYAtlasMaker : EditorWindow
{
	...
	static bool PackTextures (Texture2D tex, List<SpriteEntry> sprites)
	{
		...
		else
		{
			...
			for (int i = 0; i < sprites.Count; ++i) textures[i] = sprites[i].tex;
			rects = MYTexturePacker.PackTextures(tex, textures, 4, 4, NGUISettings.atlasPadding, maxSize);
		}
	...
	static MYAtlas.Sprite AddSprite (List<MYAtlas.Sprite> sprites, SpriteEntry se)
	{
		MYAtlas.Sprite sprite = null;

		// See if this sprite already exists
		foreach (MYAtlas.Sprite sp in sprites)
		{
			if (sp.name == se.name)
			{
				sprite = sp;
	...
	static void ReplaceSprites (MYAtlas atlas, List<SpriteEntry> sprites)
	{
		// Get the list of sprites we'll be updating
		List<MYAtlas.Sprite> spriteList = atlas.spriteList;
		List<MYAtlas.Sprite> kept = new List<MYAtlas.Sprite>();

		// The atlas must be in pixels
		atlas.coordinates = MYAtlas.Coordinates.Pixels;

		// Run through all the textures we added and add them as sprites to the atlas
		for (int i = 0; i < sprites.Count; ++i)
		{
			SpriteEntry se = sprites[i];
			MYAtlas.Sprite sprite = AddSprite(spriteList, se);
			kept.Add(sprite);
		}
		...
		{
			MYAtlas.Sprite sp = spriteList[--i];
	...
	static void ExtractSprites (MYAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (atlasTex != null)
		{
			...
			List<MYAtlas.Sprite> list = atlas.spriteList;

			foreach (MYAtlas.Sprite asp in list)
			{
				bool found = false;
	...
	static bool UpdateTexture (MYAtlas atlas, List<SpriteEntry> sprites)
	{
	...
	static public void AddOrUpdate (MYAtlas atlas, Texture2D tex)
	{
	...
	static void UpdateAtlas (MYAtlas atlas, List<SpriteEntry> sprites)
	{
		if (sprites.Count > 0)
		{
	...
	void OnGUI ()
	{
		...
		if (NGUISettings.atlas == null && go != null) NGUISettings.atlas = go.GetComponent<MYAtlas>();
		...
		if (create)
		{
				...
				NGUISettings.atlasName + " atlas with the textures currently selected in the Project View? All other sprites will be deleted.", "Yes", "No"))
			{
				...
				if (NGUISettings.atlas == null || NGUISettings.atlas.name != NGUISettings.atlasName)
				{
					...
					go.AddComponent<MYAtlas>().spriteMaterial = mat;
					...
					NGUISettings.atlas = go.GetComponent<MYAtlas>();
				}
			}
		}

		ComponentSelector.Draw<MYAtlas>("Select", NGUISettings.atlas, OnSelectAtlas);
		...
		if (spriteList.Count > 0)
		{
			...
			foreach (KeyValuePair<string, int> iter in spriteList)
			{
				...
				GUILayout.Space(-1f);
				bool highlight = (MYAtlasInspector.instance != null) && (NGUISettings.selectedSprite == iter.Key);
			...
			if (NGUISettings.atlas != null && !string.IsNullOrEmpty(selection))
			{
				...
				if (MYAtlasInspector.instance != null)
					MYAtlasInspector.instance.Repaint();
			}
		}
	}
}
