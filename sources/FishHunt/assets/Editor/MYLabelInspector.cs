...
public class MYLabelInspector : MYWidgetInspector
{
	MYLabel mLabel;
	...
	{
		mLabel = mWidget as MYLabel;
		ComponentSelector.Draw<MYFont>(mLabel.font, OnSelectFont);
		...
		if (mLabel.font != null)
		{
			...
			{
				MYFont.SymbolStyle sym = (MYFont.SymbolStyle)EditorGUILayout.EnumPopup("Symbols", mLabel.symbolStyle, GUILayout.Width(170f));
				if (sym != mLabel.symbolStyle) { RegisterUndo(); mLabel.symbolStyle = sym; }
			}

			GUILayout.BeginHorizontal();
			{
				MYLabel.Effect effect = (MYLabel.Effect)EditorGUILayout.EnumPopup("Effect", mLabel.effectStyle, GUILayout.Width(170f));
				if (effect != mLabel.effectStyle) { RegisterUndo(); mLabel.effectStyle = effect; }

				if (effect != MYLabel.Effect.None)
				{
			...
			GUILayout.BeginHorizontal();
			if (mLabel.effectStyle != MYLabel.Effect.None)
			{
