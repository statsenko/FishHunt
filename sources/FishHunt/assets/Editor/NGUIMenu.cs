	...
	static public void AddSprite ()
	{
		...
		if (go != null)
		{
			...
			if (sprite.atlas != null)
			{
				...
				MYAtlas.Sprite sp = sprite.atlas.GetSprite(sn);
				
				if (sp != null)
				{
					sprite.spriteName = sn;
					if (sp.inner != sp.outer) sprite.type = MYSprite.Type.Sliced;
				}
			}
...
*/
	...
	static public void AddAnchor ()
	{
		...
		if (go != null)
		{
			...
			if (go.GetComponent<MYAnchor>() == null) go.AddComponent<MYAnchor>();
		}
		else
		{
			Debug.Log("You must select a game object first.");
...
/*
	...
	{
		EditorWindow.GetWindow<MYCameraTool>(false, "Camera Tool", true);
	}
*/
	[MenuItem("NGUI/Open the Font Maker #&f")]
	static public void OpenFontMaker ()
	{
		EditorWindow.GetWindow<MYFontMaker>(false, "Font Maker", true);
	}
	...
	{
		EditorWindow.GetWindow<MYAtlasMaker>(false, "Atlas Maker", true);
		//w.ShowUtility ();
		//w.Focus ();
	}
	...
	{
		MYWidget.showHandlesWithMoveTool = !MYWidget.showHandlesWithMoveTool;

		if (MYWidget.showHandlesWithMoveTool)
		{
