...
public class UICreateNewUIWizard : EditorWindow
{
	...
	static public GameObject CreateNewUI ()
	{
		...
		if (camType == CameraType.Simple2D)
		{
			...
			root.AddComponent<MYRoot>().scalingStyle = MYRoot.Scaling.PixelPerfect;
		}
		else
		{
			...
			root.AddComponent<MYRoot>().scalingStyle = MYRoot.Scaling.FixedSize;
		}
...
//!!			Selection.activeGameObject = panel.gameObject;
		...
		else
		{
			...
			if (camType == CameraType.Simple2D)
			{
				...
				MYAnchor anchor = NGUITools.AddChild<MYAnchor>(cam.gameObject);
				anchor.uiCamera = cam;
